...
if [[ "$1" = "-h" ]] || [[ "$1" = "--help" ]]; then
  ...
  echo "Prints out commands to run to release callback-websocket to Maven Central."
...
if [[ "$2z" = "z" ]]; then
  ...
  new_version=$2
  echo "New development version set manually, will be ${new_version}"
fi

echo "To release callback-websocket version ${version} run these commands:"
echo ""
...
echo "git commit -m \"Prepare release callback-websocket-${version}\""
...
echo "mvn gpg:sign-and-deploy-file -Durl=https://oss.sonatype.org/service/local/staging/deploy/maven2/ -DrepositoryId=sonatype-nexus-staging -DpomFile=pom.xml -Dfile=target/callback-websocket-${version}.jar"

echo ""
echo "7) deploy and sign sources archive"
echo "mvn gpg:sign-and-deploy-file -Durl=https://oss.sonatype.org/service/local/staging/deploy/maven2/ -DrepositoryId=sonatype-nexus-staging -DpomFile=pom.xml -Dfile=target/callback-websocket-${version}-sources.jar -Dclassifier=sources"

echo ""
echo "8) deploy and sign javadoc archive"
echo "mvn gpg:sign-and-deploy-file -Durl=https://oss.sonatype.org/service/local/staging/deploy/maven2/ -DrepositoryId=sonatype-nexus-staging -DpomFile=pom.xml -Dfile=target/callback-websocket-${version}-javadoc.jar -Dclassifier=javadoc"
